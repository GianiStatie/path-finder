...
from src.utils import add_vectors, euclidean_distance
from .abstract_path_finder import AbstractPathFinder


class AStarPathFinder(AbstractPathFinder):
    """ Subclass of AbstractPathFinder
    Finds the path using A* search approach.
    ...
    def __init__(self):
        ...
        self.transition_cost_g = []
        self.transition_cost_h = []
        self.transition_node = []

    def set_graph(self, graph):
        super().set_graph(graph)
        self.transition_cost_h = self._compute_heuristics_cost(
            self.graph.get_start_node_position(), self.graph.get_all_node_positions())
        self.transition_cost_g = [
            self.default_distance] * len(self.graph.nodes)
        self.transition_cost = list(
            map(sum, zip(self.transition_cost_h, self.transition_cost_g)))
    ...
    def _compute_heuristics_cost(self, start_node, nodes):
        return [euclidean_distance(start_node, node_position)
    ...
    def _calculate_transitions(self):
        ...
        while len(unvisited_nodes) > 0:
            ...
            for connected_node in self._get_connected_nodes(current_node):
                connected_node_idx = self.graph.get_position_index(
                    connected_node)
                connected_node_weight = self.graph.get_position_weight(
                    connected_node)
                transition_cost = self.transition_cost[current_node] + \
                    connected_node_weight
                if self.transition_cost[connected_node_idx] > transition_cost:
                    self.transition_cost_g[connected_node_idx] = transition_cost
                    self.transition_cost[connected_node_idx] = transition_cost + \
                        self.transition_cost_h[connected_node_idx]
